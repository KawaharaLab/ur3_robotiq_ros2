...
def plot_for_slide(csv_dir):
    ...
    ax2.plot(df_ft['time_s'], df_ft['force_torque_right.wrench.force.y'], 
             label='Force Y', color='#2ca02c', alpha=0.7, linestyle='--', linewidth=2) # 緑
    ax2.plot(df_ft['time_s'], df_ft['force_torque_right.wrench.force.z'], 
    ...
    if len(path_parts) >= 3:
        ...
        file_name = f"slide_plot_{task_name}_{session_id}.png"
    else:
